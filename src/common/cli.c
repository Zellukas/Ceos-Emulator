...
void display_versionscreen(bool do_exit)
{
	...
	if( svn[0] != UNKNOWN_VERSION )
		ShowInfo("Ceos-Emulator SVN Revision: '"CL_WHITE"%s"CL_RESET"'\n", svn);
	else {
		const char* git = get_git_hash();
		if( git[0] != UNKNOWN_VERSION )
		ShowInfo("Ceos-Emulator Git Hash: '"CL_WHITE"%s"CL_RESET"'\n", git);
	}
	ShowInfo(CL_GREEN"Website/Forum:"CL_RESET"\thttp://Ceos-Emulator.org/\n");
	ShowInfo(CL_GREEN"IRC Channel:"CL_RESET"\tirc://irc.rizon.net/#Ceos-Emulator\n");
